...
public class Utility {
	...
	public static String getSharedString(Context context, String name,
			String key) {
	...
	public static String httpPostRequest(String url, List<NameValuePair> params) {
		...
		try {
			...
			HttpResponse httpResponse = new DefaultHttpClient()
					.execute(httpRequest);
			Debugs.d("leochin",
					"url=" + url + "--1->" + httpResponse.getStatusLine());
	 ...
	 * httpPostRequest请求，成功返回数据，失败返回为空的String
	 */
	public static void httpPostRequestNoBack(final String url,
			...
			public void run() {
				try {
					...
					httpRequest.setEntity(new UrlEncodedFormEntity(params,
							HTTP.UTF_8));
					HttpResponse rp = new DefaultHttpClient()
							.execute(httpRequest);
					Debugs.d("leochin",
							"url=" + url + "--2->" + rp.getStatusLine());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
